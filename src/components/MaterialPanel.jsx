...
export default function MaterialPanel({
  ...
  const handleDelete = (item, e) => {
    if (e) e.stopPropagation();
    setCabinetOptions((prev) => prev.filter((x) => x !== item));
    setActiveMenuIndex(null);
  ...
  const handleSaveEdit = () => {
    setCabinetOptions((prev) =>
      prev.map((x) => (x === itemToEdit._original ? itemToEdit : x))
    );
    setEditMode(false);
    setItemToEdit(null);
      ...
      <div className="space-y-4 mt-2 animate-fade-in">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              window.newProductCode = e.target.value;
            }}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="Enter product code"
          />
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              window.newManufacturer = e.target.value;
            }}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="">Select Manufacturer</option>
  ...
  const renderManageLayersPage = () => {
    // Filter + paginate table data (same as your current logic)
  ...
  <label className="inline-flex items-center cursor-pointer">
    ...
    <span
      className="
        relative w-11 h-6 rounded-full bg-slate-200 transition-colors duration-300
        peer-checked:bg-blue-600
        before:content-[''] before:absolute before:top-0.5 before:left-0.5
        before:w-5 before:h-5 before:rounded-full before:bg-white before:shadow
...
<div className="flex items-center gap-2 ml-4">
              <button className="px-3 py-2 rounded-lg bg-red-500/10 text-red-600 hover:bg-red-500/20 font-medium flex items-center gap-1">
                {/* Trash SVG */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="w-4 h-4"
              ...
              {/* Bulk Upload */}
              <button className="px-3 py-2 rounded-lg bg-blue-500 text-white hover:bg-blue-600 font-medium flex items-center gap-1">
                {/* Upload SVG */}
            ...
            {/* Right side: Delete + Bulk Upload + Close */}
                ...
                <tbody className="divide-y">
                  {pageRows.map((r, idx) => (
                    <tr key={idx} className="hover:bg-slate-50">
                      ...
                      <td className="p-3">
                        <div className="w-10 h-10 rounded-md border" style={{ backgroundColor: r.color }} />
                      </td>
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-mono">{r.color?.toUpperCase()}</span>
                        </div>
                      </td>
                      <td className="p-3">
                        <span className="px-2 py-1 rounded-full text-xs bg-slate-100">{r.designType || "Color"}</span>
                      </td>
                      <td className="p-3">
                        <span className="px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-700">{r.option || "Cabinet"}</span>
                      </td>
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          {/* <button
                            className="px-2 py-1 rounded-md border hover:bg-slate-100"
                            onClick={(e) => handleEdit(r, e)} // this already closes the page
                            title="Edit"
                          >
                            Edit
                          </button> */}
                  ...
                  {pageRows.length === 0 && (
                    <tr>
                      <td colSpan={9} className="p-6 text-center text-slate-400">No entries found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
 ...
 <div className="px-5 py-3 bg-white border-t border-slate-200 flex items-center justify-between">
      <div className="text-sm text-slate-500">
        Showing {Math.min(total, startIdx + 1)} to {Math.min(total, startIdx + pageRows.length)} of {total} entries
      </div>
      <div className="flex items-center gap-2">
        <button
        ...
        <span className="px-2 py-1 rounded-md bg-purple-600 text-white text-sm">{currentPage}</span>
        <button
          className="w-8 h-8 rounded-md border hover:bg-slate-100 disabled:opacity-50"
    ...
    <>
      {showManageLayers ? (
        ...
        >
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const newOption = {
                      ...
                      grade: "Custom",
                      image: newImage,
                      productCode: window.newProductCode || "",
                      manufacturer: window.newManufacturer || "",
                      designType: "Color",
                      option: "Door",
                    };
          ...
          {/* MANAGE LAYERS MODAL */}
          {/* {showManageLayers && renderManageLayersModal()} */}
        </div>
      )}
    </>
  );
