...
export default function MaterialPanel({ title = "Cabinets", onCancel, onConfirm, onBack, onClose, showToast }) {
  ...
  // Update renderGoogleSearchPanel to show only color and name, and add two filter icon buttons
  const renderGoogleSearchPanel = () => (
    <div className="flex flex-col flex-1"> {/* Removed h-full and added flex-1 here */}
      {/* Sticky Search and Filter Header */}
      <div className="px-2 pt-4 pb-2 bg-white z-10 sticky top-0 shadow-sm">
        <div className="flex items-center gap-2 mb-4 relative">
          <div className="relative flex items-center flex-1">
            {/* Google SVG icon */}
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-4 h-4 absolute left-3 text-slate-400"><g><path fill="#4285F4" d="M12 11v2.5h5.9c-.25 1.4-1.7 4-5.9 4-3.6 0-6.5-2.9-6.5-6.5s2.9-6.5 6.5-6.5c2.1 0 3.5.8 4.3 1.5l2.9-2.9C17.2 2.7 14.8 2 12 2 6.5 2 2 6.5 2 12s4.5 10 10 10c5.7 0 9.5-4 9.5-9.5 0-.6-.1-1.1-.2-1.5H12z"/><path fill="#34A853" d="M3.6 7.1l2.4 1.8C7.1 7.1 9.3 5.5 12 5.5c2.1 0 3.5.8 4.3 1.5l2.9-2.9C17.2 2.7 14.8 2 12 2 8.1 2 4.7 4.1 3.6 7.1z"/><path fill="#FBBC05" d="M12 22c2.7 0 5.2-.9 7.1-2.4l-2.9-2.3C14.8 18.5 13.5 19 12 19c-3.6 0-6.5-2.9-6.5-6.5 0-.7.1-1.4.3-2.1l-2.4-1.8C2.2 10.1 2 11 2 12c0 5.5 4.5 10 10 10z"/><path fill="#EA4335" d="M22 12c0-.6-.1-1.1-.2-1.5H12v3h5.9c-.25 1.4-1.7 4-5.9 4-2.5 0-4.6-1.5-5.4-3.6l-2.4 1.8C4.7 19.9 8.1 22 12 22c5.7 0 9.5-4 9.5-9.5z"/></g></svg>
            <input
              type="text"
          ...
          <div className="relative flex items-center">
           {/* Moved Region text outside button */}
            <button
              className="p-2 border gap-2  border-slate-300 rounded-lg text-slate-600 bg-slate-50 hover:bg-slate-100 transition flex items-center justify-center" // Removed gap-2
              type="button"
            ...
            >
              ...
              </svg> <span className="text-sm font-semibold text-slate-700 mr-1">Region</span> 
            </button>
            {showRegionFilter && (
              <div className="absolute right-0 px-2 top-full mt-1 w-32 bg-white border border-slate-200 rounded-lg shadow-lg z-[9999] py-1 animate-fade-in">
                {['All', 'US', 'EU', 'Asia'].map(region => (
                  <button
                    key={region}
                    className={`w-full text-left rounded-lg px-3 py-1.5 text-sm font-medium ${googleRegion === region ? 'bg-blue-100 text-blue-700' : 'hover:bg-slate-100 text-slate-700'}`}
                    onClick={() => { setGoogleRegion(region); setShowRegionFilter(false); }}
                  >
                    {region}
                  </button>
                ))}
      ...
      <div className="flex-1 max-h-[600px] overflow-y-auto scrollbar-hide p-2"> {/* Removed maxHeight, flex-1 handles height */}
        <div className="grid grid-cols-3 gap-4">
          {dummyGoogleResults
            ...
            .map((item, idx) => (
              <div key={idx} className="rounded-xl border-2 p-3 flex flex-col items-center bg-white shadow-sm">
                <div className="w-12 h-12 rounded-lg mb-2" style={{ backgroundColor: item.color }}></div>
                <div
                  title={item.name} // Added title for full name on hover
                  className="font-semibold text-slate-800 text-xs text-center mb-1 line-clamp-2" // Added line-clamp-2
                >
                  {item.name}
                </div>
              </div>
        ...
        {dummyGoogleResults.filter(item =>
          (googleRegion === "All" || item.region === googleRegion) &&
          item.name.toLowerCase().includes(googleSearchTerm.toLowerCase())
        ).length === 0 && (
